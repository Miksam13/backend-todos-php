...
const url = "http://localhost/php_projects/lvl%202/api/v1/";
const site = "http://localhost/php_projects/lvl%202/LoginToDo/login.html";
let vue = new Vue({
	el: '#app',
	data: {
		new_task: {
	...
	methods: {
		...
		getPut: function(index, id){
			let request = JSON.stringify({id: Number(id), text: this.items[index].text, checked: this.items[index].checked});
			console.log(request);
